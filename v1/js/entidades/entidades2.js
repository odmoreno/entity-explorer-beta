...
$('#collapseCardExample').on('hidden.bs.collapse', function () {
  console.log("ADIOS COLLAPSE")
  
  d3.select('#timeline').classed('showTimeline', true);
  d3.select('#timeline').classed('hideTimeline', false);

  d3.select('#lineCont').style('display', 'block')
})

$('#collapseCardExample').on('show.bs.collapse', function () {
  console.log("HOLA COLLAPSE")
  d3.select('#timeline').classed('hideTimeline', true);
  d3.select('#timeline').classed('showTimeline', false);

  d3.select('#lineCont').style('display', 'none')
})
...
createNodes = (nodos, groups) => {
  ...
  let node1 = {
    id: "nodeSi",
    numeroid: "1",
    x: groups['no'].x - Math.random(), // * (-10 - 60) + (-10),
    y: groups['no'].y - Math.random(), // * (-10 - 60) + (-10),
    r: 20,
    voto : "no",
    lastvoto : "no",
    group: "no",
